...
Console.WriteLine("dato un vettore di interi costruito su numeri casuali fare la media, il vettore contiene 10000 elementi");

int vecLenght = 100000;
double[] vec = new double[vecLenght];

Console.WriteLine("Array pipposo");
int[] vec1 = Enumerable.Range(0, vecLenght).Select(n => new Random().Next(1, 100)).ToArray();
...
{
    vec[i] = new Random().Next(1, 100);
}

double media = vec.Aggregate(0, (double old, double current) => (old + current))/vec.Length;
